...
I need one, and only one single workspace by default, since =exwm= is good enough to create those dynamically anyway, it doesn't matter much.
#+BEGIN_SRC emacs-lisp
...
If you happen to use the console terminal version of emacs, get rid of =when window-system=.
